...
void wd33c93_w(int offset, unsigned char data)
{
	switch( offset )
	{
	case 0:
...
//					timer_set( TIME_IN_HZ( 16384 ), 1, wd33c93_irq );
				...
				break;
			default:
				printf( "Unknown SCSI controller command: %02x\n", n33C93_Data);
				break;
			}
		}
...
void wd33c93_read_data(int bytes, UInt8 *pData)
{
	...
	{
		printf("wd33c93: request for unknown device SCSI ID %d\n", last_id);
	}
}
...
void *wd33c93_get_device(int id)
{
	...
	{
		printf("wd33c93: fetching dev pointer for SCSI ID %d\n", id);
