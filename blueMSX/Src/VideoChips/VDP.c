...
void vdpWriteLatch(void* dummy, UInt16 ioPort, UInt8 value)
{
    if (vdp.vdpKey) {
		if (value & 0x80) {
			if (!(value & 0x40)) {
                vdpUpdateRegisters(value, vdp.vdpDataLatch);
			}
		} 
        else {
...
void vdpInit(VdpConnector connector, VdpVersion version, VdpSyncMode sync, int vramPages)
{
    ...
    vdp.timerDisplay       = boardTimerCreate(onDisplay, NULL);
    vdp.timerDrawAreaStart = boardTimerCreate(onDrawAreaStart, NULL);
    vdp.timerVStart        = boardTimerCreate(onVStart, NULL);
    vdp.timerScrModeChange = boardTimerCreate(onScrModeChange, NULL);
    vdp.timerHint          = boardTimerCreate(onHint, NULL);
    vdp.timerVint          = boardTimerCreate(onVint, NULL);
    ...
    case VDP_SVI:
        ioPortRegister(0x80, NULL,          vdpWrite,      NULL); // vdp.vdpRegs vdp.vram Write
        ioPortRegister(0x81, NULL,          vdpWriteLatch, NULL); // vdp.vdpRegs Address Latch
        ioPortRegister(0x84, vdpRead,       NULL,          NULL); // vdp.vdpRegs vdp.vram Read
        ioPortRegister(0x85, vdpReadStatus, NULL,          NULL); // vdp.vdpRegs Status Read
        break;
